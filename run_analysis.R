...
setwd('C:/Users/Ubirajara/Downloads/Cursos Coursera 2015/Project');
...
# Create a new table, finalDataNoActivityType (FDNAT) without the activityType column
FDNAT  = finalData[,names(finalData) != 'activityType'];

# Summarizing the FDNAT table to include just the mean of each variable for each activity and each subject
tidyData    = aggregate(FDNAT[,names(FDNAT) != c('activityId','subjectId')],by=list(activityId=FDNAT$activityId,subjectId = FDNAT$subjectId),mean);
